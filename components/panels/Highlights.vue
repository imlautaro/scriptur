...
<template>
	<Stack vertical>
		<Stack class="p-3 border-b bg-white" items="center" justify="between">
			<h1 class="text-lg font-medium ml-2">{{ $t('highlights') }}</h1>
			<div class="relative">
				<IconButton
		...
		<Stack v-if="showFilters" class="p-4 border-b" vertical>
			...
			>
				...
				<optgroup :label="$t('old') + ' ' + $t('testament')">
					<option v-for="book in oldTestamentBooks" :value="book.id">
						{{ $t(`books.titles.${book.key}`) }}
					</option>
				</optgroup>
				<optgroup :label="$t('new') + ' ' + $t('testament')">
					<option v-for="book in newTestamentBooks" :value="book.id">
						{{ $t(`books.titles.${book.key}`) }}
					</option>
				</optgroup>
			</select>
