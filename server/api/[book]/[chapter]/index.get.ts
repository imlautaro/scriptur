...
export default defineCachedEventHandler(
	async event => {
		...
		const getVersesWithComments = async (book: number, chapter: number) => {
		...
		try {
			...
			const versesWithComments = await getVersesWithComments(
				currentBook,
				currentChapter
			)
			...
			console.log(
				verses.map(verse => ({
					...verse,
					hasComments: versesWithComments.includes(verse.verse),
				}))
			)
